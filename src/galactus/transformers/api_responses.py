...
domain's API transformer fn and the list of sources it covers.
"""
...
def _transform_row(row: dict, transformer_fn: Callable) -> list[dict] | None:
    response_text = decompress(bytes(row["response_blob"]))
    try:
        return transformer_fn(row["source"], response_text)
    except Exception:
        logger.exception("Failed to transform api_response %d (%s)", row["id"], row["endpoint"])
        return None
