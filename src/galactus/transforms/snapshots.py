...
domain's HTML parser fn and the list of sources it covers.
"""
...
async def run(
    *,
    ...
    parser_fn: Callable[[str, str, str], dict | None],
...
def _parse_row(row: dict, parser_fn: Callable) -> dict | None:
    html = decompress(bytes(row["html_blob"]))
    try:
        return parser_fn(row["source"], html, row["url"])
    except Exception:
