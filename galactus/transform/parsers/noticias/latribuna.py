...
class Parser(BaseParser, ArticleParser):
    ...
    The bronze body is an Arc Publishing PF ``story-feed-query`` page;
    each ``content_elements`` entry is a story object whose body lives
    in its own nested ``content_elements``. build_item splits the outer
    feed into one item per Article.
    """
