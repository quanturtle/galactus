...
class Parser(BaseParser):
    """Parses ApiSnapshots from lanacion.com.py into Article entities.

    The bronze body is an Arc Publishing PF ``content-search-feed-full``
    page; each ``content_elements`` entry is a story object whose body lives
    in its own nested ``content_elements``. Stories without a resolvable
    headline are skipped (silver.articles requires one).
    """

    bronze_model = ApiSnapshot
    silver_model = Article

    SITE_BASE = "https://www.lanacion.com.py"
