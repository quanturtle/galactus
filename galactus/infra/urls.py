...
def normalize(url: str, strip_path_prefixes: list[str] | None = None) -> str:
    """Normalize a URL: force https, add www., sort query params, strip trailing slash."""
    parsed = urlparse(url)
...
def extract_same_domain_links(html: str, page_url: str, home_domain: str) -> list[str]:
    """Extract all same-domain href links from an HTML page."""
    ...
    for a in soup.find_all("a", href=True):
        ...
        if f".{ext}" in SKIP_EXTENSIONS:
            continue
        # skip broken relative links that produce consecutive duplicate path segments
